...
1. WHEN 사용자가 사이드바를 볼 때 THEN 시스템 SHALL "마켓" 섹션(`sectionMarket`)에 "마켓 스크리너" 메뉴 항목을 적절한 아이콘과 함께 표시한다.
...
2. WHEN 데이터가 로드된 후 THEN 시스템 SHALL 다음 컬럼이 포함된 테이블을 표시한다: 코인명(심볼+아이콘), 가격(USD), 24h 변화율(%), 24h 거래량(USD, 약식 표기 $1.2B), OI(USD, 약식 표기), 펀딩비율(%).
3. WHEN 24h 변화율이 양수일 때 THEN 시스템 SHALL 해당 셀을 녹색(상승)으로 표시한다.
4. WHEN 24h 변화율이 음수일 때 THEN 시스템 SHALL 해당 셀을 빨간색(하락)으로 표시한다.
5. WHEN 펀딩비율이 양수일 때 THEN 시스템 SHALL 해당 셀을 녹색으로, 음수일 때 빨간색으로 표시한다.
6. WHEN 여러 거래소에 동일 코인이 존재할 때 THEN 시스템 SHALL 거래소별 데이터를 하나의 행으로 집계(가격: 거래량 가중 평균, 거래량/OI: 합산, 펀딩비율: OI 가중 평균)한다.
7. WHEN 사용자가 테이블 헤더 컬럼을 클릭할 때 THEN 시스템 SHALL 해당 컬럼 기준으로 오름차순/내림차순 정렬을 토글한다.
8. WHEN 사용자가 테이블의 코인 행을 클릭할 때 THEN 시스템 SHALL `/futures-dashboard?coin=XXX` 경로로 이동하여 해당 코인의 상세 대시보드를 표시한다.
9. WHEN 데이터 로딩 중일 때 THEN 시스템 SHALL 스켈레톤 UI를 표시하여 사용자에게 로딩 상태를 알린다.
10. WHEN 거래소 API 호출이 실패할 때 THEN 시스템 SHALL 실패한 거래소를 제외한 나머지 데이터로 테이블을 표시하고, 일부 거래소 데이터가 누락됨을 안내한다.
...
5. WHEN 사용자가 "New Listings" 탭을 선택할 때 THEN 시스템 SHALL 최근 거래소에 신규 상장된 코인 목록을 상장일 기준 최신순으로 표시한다.
6. IF "New Listings" 데이터를 아직 수집하지 못한 경우 THEN 시스템 SHALL 해당 탭에 "데이터 준비 중" 안내 메시지를 표시한다.
7. WHEN 새로 상장된 코인이 감지되면 THEN 시스템 SHALL 해당 코인에 "NEW" 배지를 표시한다.
8. WHERE New Listings 기준 THEN 시스템 SHALL 최근 30일 이내에 상장된 코인을 신규 상장으로 분류한다.
...
1. WHEN 페이지가 로드될 때 THEN 시스템 SHALL Recharts를 사용하여 수익률 분포 히스토그램을 스크리너 테이블 하단에 표시한다.
...
6. WHEN 사용자가 히스토그램 막대 위에 마우스를 올릴 때(hover) THEN 시스템 SHALL 해당 구간에 속하는 코인 목록(심볼 + 수익률)을 툴팁으로 표시한다.
...
10. IF 1w 또는 1m 기간이 선택된 경우 THEN 시스템 SHALL 추가적인 Kline API 호출을 통해 해당 기간의 변화율을 산출한다.
...
1. WHEN 페이지가 로드될 때 THEN 시스템 SHALL Recharts를 사용하여 거래소별 24h 총 선물 거래량(USD)을 바 차트로 표시한다.
...
5. WHEN 차트가 표시되면 THEN 시스템 SHALL 거래량이 큰 거래소 순서로 막대를 정렬한다.
...
1. WHEN 페이지가 로드될 때 THEN 시스템 SHALL Recharts를 사용하여 거래소별 현재 총 OI(USD)를 바 차트로 표시한다.
2. WHERE 바 차트 THEN 시스템 SHALL 6개 거래소를 각각 다른 색상(Market Volume 차트와 동일한 색상 스킴)으로 구분하여 표시한다.
...
5. IF 특정 거래소의 벌크 ticker API에 OI가 포함되지 않은 경우(예: Binance는 별도 API `/fapi/v1/openInterest` 필요) THEN 시스템 SHALL 가능한 데이터 소스를 사용하거나 해당 거래소를 "데이터 없음"으로 표시한다.
6. WHEN 차트가 표시되면 THEN 시스템 SHALL OI가 큰 거래소 순서로 막대를 정렬한다.
...
1. WHEN 페이지가 로드될 때 THEN 시스템 SHALL Recharts를 사용하여 6개 섹터(DeFi, L1, L2, Gaming, AI, Meme)의 평균 수익률을 비교 차트로 표시한다.
2. WHEN 기간 선택기가 표시될 때 THEN 시스템 SHALL 1d, 1w, 1m 세 가지 기간 옵션을 제공한다.
...
5. WHEN 사용자가 차트의 특정 섹터 위에 마우스를 올릴 때 THEN 시스템 SHALL 해당 섹터의 평균 수익률, 포함된 코인 수, 구성 코인 목록을 툴팁으로 표시한다.
6. WHERE 양의 수익률 섹터 THEN 시스템 SHALL 녹색 계열로, 음의 수익률 섹터 SHALL 빨간색 계열로 표시한다.
7. WHEN 차트가 표시되면 THEN 시스템 SHALL 성과가 높은 섹터 순서로 정렬한다.
...
4. WHEN 동일 코인에 대해 여러 거래소 데이터가 존재할 때 THEN 시스템 SHALL 거래소별 데이터를 개별 보존하면서 집계 데이터(가중 평균 가격, 합산 거래량/OI, OI 가중 평균 펀딩비율)도 함께 제공한다.
5. WHEN Route Handler가 호출될 때 THEN 시스템 SHALL 응답에 적절한 Cache-Control 헤더(예: `s-maxage=30, stale-while-revalidate=60`)를 설정하여 불필요한 중복 호출을 방지한다.
...
**사용자 스토리:** 개발자로서, 거래소마다 다른 심볼 형식을 통합적으로 관리하고 싶다. 이를 통해 동일 코인의 데이터를 거래소 간에 정확하게 매칭할 수 있다.

#### 수락 기준

1. WHERE 심볼 정규화 THEN 시스템 SHALL 각 거래소의 심볼 형식을 공통 형식(예: "BTC", "ETH")으로 변환한다:
   - Binance: "BTCUSDT" -> "BTC"
   - Bybit: "BTCUSDT" -> "BTC"
   ...
   - Bitget: "BTCUSDT" -> "BTC"
   - Hyperliquid: "BTC" -> "BTC"
2. WHEN 정규화된 심볼이 동일한 경우 THEN 시스템 SHALL 해당 코인의 거래소별 데이터를 동일 코인으로 매칭한다.
3. IF USDT-마진 외의 선물(COIN-마진 등)이 포함되면 THEN 시스템 SHALL USDT-마진 선물만 필터링한다.

---

### 요구사항 14: 정적 매핑 데이터 관리
...
6. WHERE 매핑 파일 THEN 시스템 SHALL 250개 이상의 주요 선물 거래 코인에 대한 분류를 포함한다.

---

### 요구사항 15: New Listings 감지

**사용자 스토리:** 트레이더로서, 최근 거래소에 새로 상장된 선물 코인을 빠르게 확인하고 싶다. 이를 통해 신규 상장 코인의 초기 거래 기회를 놓치지 않을 수 있다.

#### 수락 기준

1. WHERE New Listings 감지 THEN 시스템 SHALL 거래소의 instrument 정보 API를 호출하여 상장 정보를 수집한다:
...
2. WHEN 신규 상장 코인이 감지되면 THEN 시스템 SHALL 해당 코인의 상장일 정보를 함께 저장한다.
3. WHERE New Listings 기준 THEN 시스템 SHALL 최근 30일 이내에 상장된 코인을 신규 상장으로 분류한다.
4. WHEN "New Listings" 탭이 선택되면 THEN 시스템 SHALL 최근 상장된 코인을 상장일 기준 최신순으로 정렬하여 표시한다.

---

### 요구사항 16: 클라이언트 상태 관리 및 갱신
...
2. WHILE 사용자가 페이지에 머무르는 동안 THEN 시스템 SHALL 60초 주기로 데이터를 자동 refetch한다.
...
5. WHEN 페이지가 백그라운드(탭 비활성)일 때 THEN 시스템 SHALL 불필요한 API 호출을 방지하기 위해 자동 갱신을 일시 중지한다.
6. WHEN 데이터 갱신 시 가격 변동이 발생한 경우 THEN 시스템 SHALL 변동된 셀에 짧은 하이라이트 애니메이션(flash)을 적용하여 변동을 시각적으로 알린다.
7. WHEN 사용자가 수동 새로고침 버튼을 클릭하면 THEN 시스템 SHALL 즉시 모든 거래소 데이터를 갱신한다.
8. WHEN 마지막 갱신 시간이 2분 이상 경과하면 THEN 시스템 SHALL 데이터가 오래되었음을 시각적으로(경고 배지 등) 표시한다.
...
### 요구사항 18: 페이지 레이아웃 및 반응형 디자인

**사용자 스토리:** 사용자로서, 다양한 화면 크기에서 마켓 스크리너를 편리하게 사용하고 싶다.

#### 수락 기준

1. WHEN 데스크톱(lg 이상, 1024px+) 화면에서 볼 때 THEN 시스템 SHALL 스크리너 테이블을 상단에, 하단에 차트 위젯들(Return Buckets, Market Volume, Total OI, Sector Performance)을 2x2 그리드로 배치한다.
2. WHEN 태블릿(md, 768px~1023px) 화면에서 볼 때 THEN 시스템 SHALL 차트 위젯들을 2열 그리드로 표시한다.
3. WHEN 모바일(sm 이하, 767px-) 화면에서 볼 때 THEN 시스템 SHALL 차트 위젯들을 1열로 스택하여 표시한다.
4. WHERE 테이블 THEN 시스템 SHALL 수평 스크롤을 지원하여 모바일에서도 모든 컬럼을 확인할 수 있게 한다.
5. WHERE 기존 BitScope 디자인 THEN 시스템 SHALL 기존 테마(다크 모드 포함)와 일관된 스타일(Tailwind CSS, shadcn/ui)을 적용한다.
6. WHERE 페이지 타이틀 THEN 시스템 SHALL "Market Screener" 제목과 마지막 데이터 갱신 시간을 표시한다.
...
5. WHEN 차트 컴포넌트가 필요할 때 THEN 시스템 SHALL 동적 임포트(lazy loading)를 사용하여 초기 번들 크기를 최소화한다.

### NFR-2: 에러 핸들링 및 복원력

1. WHEN 개별 거래소 API가 실패할 때 THEN 시스템 SHALL 나머지 거래소 데이터로 서비스를 계속 제공한다(Graceful Degradation).
2. WHEN 거래소 API 호출이 실패할 때 THEN 시스템 SHALL 페이지 상단에 어떤 거래소 데이터가 누락되었는지 경고 배너를 표시한다.
3. WHEN 모든 거래소 API가 실패할 때 THEN 시스템 SHALL 에러 메시지와 재시도 버튼을 표시한다.
4. WHEN API 호출이 타임아웃될 때 THEN 시스템 SHALL 5초 후 해당 거래소를 실패로 처리한다.
5. WHEN 데이터 갱신 중 에러가 발생하면 THEN 시스템 SHALL 이전 캐시 데이터를 유지하면서 에러 상태를 사용자에게 표시한다.
6. WHERE Next.js Route Handler THEN 시스템 SHALL 거래소 API 에러 응답(4xx, 5xx)을 적절한 에러 메시지로 변환하여 프론트엔드에 전달한다.
...
| 항목 | 요구사항 |
|---|---|
| **성능** | 초기 로드 3초 이내, 탭 전환 100ms 이내, 250+ 코인 가상 스크롤, 차트 lazy loading |
| **가용성** | 개별 거래소 장애 시 부분 동작 보장 (Graceful Degradation) |
| **데이터 신선도** | 60초 자동 갱신, 2분 초과 시 경고 표시, 탭 비활성 시 갱신 중지 |
| **호환성** | 데스크톱/태블릿/모바일 반응형 지원, 다크 모드 |
| **접근성** | 시맨틱 HTML, ARIA 레이블, 키보드 탐색, WCAG AA |
| **국제화** | 한국어/영어 전환, 숫자 축약 표기 |
| **기술 스택** | Next.js Route Handler, TanStack Query, Recharts, Tailwind CSS, shadcn/ui |
| **Rate Limit** | 각 거래소 API rate limit 준수 |
| **데이터 정확성** | 6개 거래소 벌크 ticker API 병렬 호출, 심볼 정규화를 통한 정확한 매칭 |
