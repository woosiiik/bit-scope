...
3. WHEN 수집이 완료되면 THEN 시스템 SHALL 각 데이터 포인트를 `funding_oi_snapshot` 테이블에 (timestamp, symbol, exchange, fundingRate, openInterest) 형태로 배치 인서트한다.
...
7. IF 각 거래소 API의 심볼 포맷이 다른 경우(예: Binance "BTCUSDT", OKX "BTC-USDT-SWAP", Gate.io "BTC_USDT") THEN 시스템 SHALL 통일된 기본 심볼(예: "BTC")로 정규화하여 저장한다.
8. IF TypeORM Entity에 `idx_funding_oi_symbol_time` 인덱스가 정의되어 있으면 THEN DB SHALL symbol과 timestamp 기준으로 효율적인 시계열 쿼리를 지원한다.
9. WHEN 각 수집 사이클이 완료될 때 THEN 시스템 SHALL 수집 건수, 소요 시간, 실패 거래소 등의 요약 로그를 남겨야 한다.
...
2. WHEN 히트맵을 표시할 때 THEN 시스템 SHALL OI 가중 평균 펀딩을 계산한다: `Sigma(거래소i의 Funding x 거래소i의 OI) / Sigma(거래소i의 OI)`.
...
5. WHEN 히트맵의 특정 셀에 마우스를 호버하면 THEN 시스템 SHALL 해당 코인, 시간, OI 가중 평균 펀딩 비율(%), 각 거래소별 펀딩 비율을 툴팁으로 표시한다.
6. WHEN 히트맵에 표시할 코인이 많을 경우 THEN 시스템 SHALL OI 상위 N개(기본 30개) 코인만 표시하고, 나머지는 스크롤로 접근할 수 있어야 한다.
7. WHEN 데이터를 조회할 때 THEN Next.js Route Handler SHALL apps/api의 집계 API를 프록시하고, 1분 TTL 캐시를 적용한다.
8. WHEN 서버에서 데이터가 아직 수집되지 않았으면 THEN 시스템 SHALL "데이터 수집 중입니다. 잠시 후 다시 시도해주세요." 메시지를 표시한다.
...
5. IF 수집 대상 코인이 너무 많아 Rate Limit에 걸릴 위험이 있는 경우 THEN 시스템 SHALL 코인 간 적절한 딜레이(예: 100ms)를 적용하거나 벌크 API를 우선 사용한다.
6. WHEN API 호출이 실패하면 THEN 시스템 SHALL Requirement 1과 동일한 에러 처리 및 백오프 전략을 적용한다.
...
2. WHEN CVD를 계산할 때 THEN 시스템 SHALL 다음 공식을 사용한다: `CVD = Sigma(takerBuyVolume - takerSellVolume)` (선택 기간 내 누적).
3. WHEN Normalized CVD를 계산할 때 THEN 시스템 SHALL 다음 공식을 사용한다: `Normalized CVD = CVD / 전 거래소 OI 합산`.
4. WHEN 차트를 렌더링할 때 THEN 시스템 SHALL 상위/하위 20개 코인의 Normalized CVD를 수평 바 차트로 표시한다 (양수 = 순매수 우세 녹색, 음수 = 순매도 우세 빨강).
5. WHEN 차트 바에 마우스를 호버하면 THEN 시스템 SHALL 해당 코인의 Normalized CVD 값, 원시 CVD(USD), 전체 OI(USD)를 툴팁으로 표시한다.
6. IF 전 거래소 OI 합산이 0이면 THEN 시스템 SHALL 해당 코인을 목록에서 제외하여 0 나누기 오류를 방지한다.
...
3. WHEN 차트를 렌더링할 때 THEN 시스템 SHALL X축을 시간, Y축을 Annualized Basis(%)로 구성한 LineChart(Recharts)를 표시한다.
4. IF BTC/ETH 외의 코인이 선택되면 THEN 시스템 SHALL "이 코인은 3M Basis를 지원하지 않습니다" 메시지를 표시한다 (기존 동작 유지).
...
6. IF 요청한 기간의 데이터가 아직 충분히 수집되지 않은 경우 THEN 시스템 SHALL 가용한 데이터 범위만 반환하고 `dataRange` 필드에 실제 데이터 범위를 명시한다.
7. WHEN 집계 쿼리 실행 시간이 5초를 초과하면 THEN 시스템 SHALL 타임아웃 에러를 반환한다.
8. WHEN API 응답을 반환할 때 THEN 시스템 SHALL `{ success: boolean, data: T, timestamp: number }` 형식의 일관된 응답 구조를 사용한다 (기존 LiquidationController 패턴 준수).
...
7. WHEN 쿼리 파라미터를 전달할 때 THEN Route Handler SHALL period, symbol 등 필요한 파라미터를 apps/api로 그대로 전달한다.
...
5. WHEN 동일 시간대에 같은 거래소/심볼의 데이터가 중복 수집되면 THEN 시스템 SHALL 중복 인서트를 방지하거나 upsert로 처리한다.
6. WHEN 데이터 보존 기간(90일)이 경과하면 THEN 시스템 SHALL 매일 1회(@Cron) 오래된 레코드를 자동 삭제한다.
7. WHEN 배치 인서트를 수행할 때 THEN 시스템 SHALL TypeORM의 `insert()` 메서드를 사용하여 일괄 저장하고, 개별 insert 반복을 피한다.
...
5. WHERE 새로운 거래소를 추가할 때 THEN 시스템 SHALL 기존 코드에 최소한의 변경으로 새 거래소를 추가할 수 있는 구조(전략 패턴 또는 인터페이스 기반)를 제공한다.
...
7. IF 특정 거래소가 연속 5회 이상 실패하면 THEN 시스템 SHALL 경고(WARN) 수준의 로그를 남겨야 한다.
