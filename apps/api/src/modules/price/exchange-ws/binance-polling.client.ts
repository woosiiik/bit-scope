/**
 * 바이낸스 REST 폴링 시세 클라이언트
 *
 * 바이낸스 공개 REST API를 주기적으로 폴링하여 USDT 마켓 시세 데이터를 수신한다.
 * 국내 거래소와의 김치 프리미엄 비교를 위한 해외 시세 수집 전용 클라이언트이다.
 *
 * 바이낸스 API는 공개 API이므로 인증이 불필요하다.
 * ExchangeType에 포함되지 않는 별도의 폴링 클라이언트로 구현한다.
 *
...
/** 바이낸스 시세 항목 (정규화 후) */
export interface BinancePriceEntry {
  /** 코인 심볼 (예: "BTC") - USDT 마켓 기준으로 USDT 접미사 제거 */
  symbol: string;
  /** USDT 가격 */
  usdtPrice: number;
  /** 타임스탬프 (밀리초) */
...
export class BinancePollingClient extends EventEmitter implements OnModuleDestroy {
  ...
  /** 구독 중인 심볼 목록 (바이낸스 USDT 마켓 기준, 예: ["BTC", "ETH"]) */
  private subscribedSymbols: string[] = [];

  /** 내부 가격 맵: 심볼 -> BinancePriceEntry */
  private readonly priceMap = new Map<string, BinancePriceEntry>();
  ...
  subscribe(symbols: string[]): void {
    ...
    this.logger.log(`바이낸스 심볼 구독 추가: [${newSymbols.join(', ')}]`);
  }

  /**
   * 특정 심볼의 현재 바이낸스 USDT 가격을 조회한다.
   *
   * @param symbol 코인 심볼 (예: "BTC")
   * @returns 바이낸스 가격 정보 또는 null
   */
  getPrice(symbol: string): BinancePriceEntry | null {
   ...
   * 전체 바이낸스 가격 맵을 반환한다.
   */
  getAllPrices(): Map<string, BinancePriceEntry> {
  ...
  private startPolling(): void {
    ...
    this.logger.log(
      `바이낸스 REST 폴링 시작 - 간격: ${this.pollingIntervalMs}ms`,
    );
  }
  ...
  /**
   * 바이낸스 REST API에서 전체 시세 데이터를 조회한다.
   *
  ...
  private async fetchTickers(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `바이낸스 API 응답 오류: ${response.status} ${response.statusText}`,
        );
      }

      const data = (await response.json()) as BinanceTickerPriceItem[];
      ...
      const subscribedSet = new Set(
        this.subscribedSymbols.map((s) => `${s.toUpperCase()}USDT`),
      );
      ...
      for (const item of data) {
        ...
        const usdtPrice = parseFloat(item.price);
