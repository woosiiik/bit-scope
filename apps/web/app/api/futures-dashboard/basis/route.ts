...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  const cacheKey = buildCacheKey('fd', 'basis', { symbol, period });
  ...
  try {
    const url = `${getApiBaseUrl()}/phase2/basis?symbol=${encodeURIComponent(symbol)}&period=${encodeURIComponent(period)}`;
