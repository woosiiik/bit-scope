/**
 * 선물 오픈오더 조회 릴레이 Route Handler
 *
 * 클라이언트가 서명한 거래소 선물 오픈오더 조회 요청을 거래소 API에 릴레이하고,
 * 응답을 통일된 FuturesOpenOrder[] 타입으로 정규화하여 반환한다.
 *
 * 지원 거래소: Binance, Gate.io, Bitget
 *
 * - POST 메서드: 서명된 요청을 통한 선물 오픈오더 조회
...
import { normalizeFuturesOpenOrders } from '../../_lib/normalizer';

/** 선물 오픈오더 지원 거래소 */
const FUTURES_ORDER_EXCHANGES: readonly string[] = ['binance', 'bybit', 'okx', 'gate', 'bitget', 'hyperliquid'] as const;
...
export async function POST(
  ...
  // 거래소 식별자 유효성 검증
  if (!FUTURES_ORDER_EXCHANGES.includes(exchangeParam)) {
    return NextResponse.json(
      {
        success: false,
        error: {
          message: `선물 오픈오더를 지원하지 않는 거래소입니다: ${exchangeParam}`,
  ...
  if (exchange === 'hyperliquid') {
    return handleHyperliquidOpenOrders(signedRequest);
  }
  ...
  const proxyResponse = await relayRequest({
    exchange,
    signedRequest,
    cacheEndpoint: EXCHANGE_ENDPOINTS[exchange].futuresOpenOrders,
  });
  ...
  // 응답 데이터 정규화
  try {
    const openOrders = normalizeFuturesOpenOrders(exchange, proxyResponse.data);
...
/**
 * 하이퍼리퀴드 선물 오픈오더 조회 (직접 호출)
 *
 * 하이퍼리퀴드는 서명이 불필요하므로 POST /info에 직접 호출한다.
 * type: "openOrders"로 미체결 주문 목록을 조회한다.
 */
async function handleHyperliquidOpenOrders(signedRequest: SignedRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const openOrders = normalizeFuturesOpenOrders('hyperliquid', rawData);

    return NextResponse.json({
      success: true,
      data: { exchange: 'hyperliquid', openOrders, timestamp: Date.now() },
      cached: false,
    });
  } catch (error) {
