/**
 * Bitget 거래소 요청 서명 모듈 (BitgetSigner)
 *
 * Bitget API v2 인증 방식에 따라 HMAC-SHA256 + Base64 서명을 생성하고,
 * 서명된 요청을 구성한다.
 *
 * Bitget 인증 방식 (OKX와 거의 동일):
 * - ACCESS-KEY 헤더: API Key
 * - ACCESS-SIGN 헤더: HMAC-SHA256 서명 (Base64 인코딩)
 * - ACCESS-TIMESTAMP 헤더: 타임스탬프 (밀리초 문자열)
 * - ACCESS-PASSPHRASE 헤더: Passphrase
 * - 서명 문자열: timestamp + method + requestPath + queryString + body
 *
 * Passphrase 처리:
 * - Bitget은 OKX와 동일하게 API Key 외에 Passphrase가 추가로 필요하다.
 ...
 * - 타임스탬프 기반으로 서명하여 서명된 요청이 일회성이 되도록 한다.
 *
...
import { BITGET_CONFIG, BITGET_ENDPOINTS } from '@bitscope/shared';

/** Bitget secretKey 내 passphrase 구분자 (OKX와 동일) */
 ...
 *
 * Bitget API는 밀리초 문자열 형식의 타임스탬프를 요구한다.
 *
 ...
 *
 * Bitget은 OKX와 동일하게 서명 결과를 Base64로 인코딩한다.
 * 서명 문자열: timestamp + method + requestPath + body
 *
 * @param preSignString 서명 대상 문자열
 * @param secretKey Bitget API Secret Key (실제 키, passphrase가 분리된 상태)
...
/**
 * Bitget 거래소 API 요청에 대한 서명을 생성한다.
 *
 * 요청 파라미터를 기반으로 HMAC-SHA256 + Base64 서명을 생성하고,
 * ACCESS-KEY, ACCESS-SIGN, ACCESS-TIMESTAMP, ACCESS-PASSPHRASE 헤더를
...
export function signRequest(params: SignRequestParams): SignedRequest {
  ...
  if (!apiKey.accessKey) {
    throw new Error('Bitget API Key가 필요합니다.');
  }
  if (!apiKey.secretKey) {
    throw new Error('Bitget Secret Key가 필요합니다.');
  }
  ...
  if (!passphrase) {
    throw new Error('Bitget Passphrase가 필요합니다. Secret Key를 "secretKey|||passphrase" 형식으로 입력해주세요.');
  }

  const timestamp = generateTimestamp();

  // requestPath 구성 (endpoint + queryString)
  // Bitget 공식 문서: queryString이 있으면 "?" 포함하여 requestPath에 연결
  ...
  // 서명 문자열: timestamp + METHOD + requestPath + body
  // Bitget 공식 문서: method는 대문자 (GET, POST)
  ...
  // URL 구성
  const baseUrl = BITGET_CONFIG.restBaseUrl;
...
/**
 * Bitget API Key의 유효성을 검증한다.
 *
 * 잔고 조회 API(/api/v2/spot/account/assets)를 호출하여
 * API Key가 유효한지 확인한다.
...
export async function validateApiKey(apiKey: ApiKeyPair): Promise<ApiKeyValidationResult> {
  try {
    ...
    const signed = signRequest({
      method: 'GET',
      endpoint: BITGET_ENDPOINTS.balance,
      apiKey,
    });

    // Next.js Route Handler를 통해 프록시 호출
    const response = await fetch('/api/exchange/bitget/balance', {
      method: 'POST',
